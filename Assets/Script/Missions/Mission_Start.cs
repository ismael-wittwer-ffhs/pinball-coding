...
public class Mission_Start : MonoBehaviour
{
    ...
    public bool b_Mission_Multi_Timer; // init timer after hitting object

    [Header("--> Options during mission", order = 0)] // --> Options during mission
    [Header("Timer", order = 1)] // --> Timer options during mission
    public bool b_Mission_Timer; // Only one timer during a mission
    ...
    public bool b_SkillShot; // true if it is the skillshot mission

    public bool BallSaver; // Player win a ball Saver
    public bool BeginWithKickBack; // If you want a mission start with the kickback open
    public bool ExtraBall; // Player win an extra ball

    [Header("Type : Bumper")] // --> 
    public bool Grp_1_Bumper;

    [Header("Type : Hole")] // --> 
    public bool Grp_1_Hole;

    [Header("Type : Rollover")] // --> 
    public bool Grp_1_Rollover;

    [Header("Type : Spinner")] // --> 
    public bool Grp_1_Spinner;

    [Header("Type : Target", order = 1)] // --> 
    public bool Grp_1_Target = true;

    [Header("Type : Bumper")] // --> 
    public bool Grp_2_Bumper;

    [Header("Type : Hole")] // --> 
    public bool Grp_2_Hole;

    [Header("Type : Rollover")] // --> 
    public bool Grp_2_Rollover;

    [Header("Type : Spinner")] // --> 
    public bool Grp_2_Spinner;
    ...
    public bool KeepLedGrp1OnDuringMission;

    public bool KickBack; // When player win the mission one or more doors are open
    ...
    public bool multiBall; // if multi ball activate

    public bool Multiplier; // Increase the multiplier Bonus x2 x4 x6 x8 x10 Super Bonus (Choose the Multiplier_SuperBonus value On the gameObject Manger_Game on the hierarchy)

    [Header("Random Bonus between (ExtraBall,BallSaver,Multiplier,Points)")] // 
    public bool Random_Bonus; // Random bonus between : ExtraBall, BallSaver, Multiplier, KickBack

    public bool Rollover_No_Order_Grp_1; // Check if there is no particular order to shoot the target
    public bool Rollover_No_Order_Grp_2; // Check if there is no particular order to shoot the target
    public bool Rollover_Order_Grp_1; // Check if there is particular order to shoot the target. The target order is the same as the  obj_Grp_1 array order
    public bool Rollover_Order_Grp_2; // Check if there is particular order to shoot the target. The target order is the same as the  obj_Grp_1 array order
    public bool Rollover_Type3_Grp_1; // a group of rollover that could be change when the player press flippers buttons
    public bool SpecificText; // If you want a specific text appear letter by letter on LCD Screen. Use for rollover Grp1
    ...
    public bool Target_Order_Grp_1; // Check if there is particular order to shoot the target. The target order is the same as the  obj_Grp_1 array order
    public bool Target_Order_Grp_2; // Check if there is particular order to shoot the target. The target order is the same as the  obj_Grp_1 array order
    public bool Target_Type_Stationary;
    ...
    public int LCD_AnimNumComplete; // if LCD_AnimNumComplete = -1 no animation is playing/
    public int LCD_AnimNumFail; // if LCD_AnimNumFail = -1 no animation is playing/
    public int LCD_AnimNumPart1; // if LCD_AnimNumPart1 = -1 no animation is playing/
    public int LCD_AnimNumPart2; // if LCD_AnimNumPart2 = -1 no animation is playing/
    public int LCD_AnimNumPart3; // if LCD_AnimNumPart3 = -1 no animation is playing/
    public int LED_Anim_Num_Complete; // if LED_Anim_Num_Complete = -1 no animation is playing/
    public int LED_Anim_Num_Fail; // if AniNumberFail = -1 no animation is playing/
    ...
    public int LED_Anim_Num_Part1; // if LED_Anim_Num_Part1 = -1 no animation is playing/

    public int LED_Anim_Num_Part2; // if LED_Anim_Num_Part2 = -1 no animation is playing/
    public int LED_Anim_Num_Part3; // if LED_Anim_Num_Part3 = -1 no animation is playing/
    public int Mission_Time = 10;
    public int numberOfBall = 3; // number of ball for the multi-ball
    public int PF_AnimNumComplete; // if PF_AnimNumComplete = -1 no animation is playing/
    public int PF_AnimNumFail; // if PF_AnimNumFail = -1 no animation is playing/
    ...
    public int PF_AnimNumPart1; // if PF_AnimNumPart1 = -1 no animation is playing/

    public int PF_AnimNumPart2; // if PF_AnimNumPart2 = -1 no animation is playing/
    public int PF_AnimNumPart3; // if PF_AnimNumPart3 = -1 no animation is playing/
    ...
    public List<int> arr_led_State = new();
    ...
    public List<int> arr_obj_Grp_1_State = new();
    public List<int> arr_obj_Grp_2_State = new();
    ...
    private bool a_Sound_is_Playing_When_Pause_Game_Start;
    private bool b_Mission_SkillShot; // use to enable or disable skillshot mission
    private bool b_MissionTimer; // use for the mission timer
    private bool b_Pause; // Used to pause the mission
    private bool b_Skillshot_OnAir; // Use to prevent the skillshot timer restart if the ball is not correctly ejected from the plunger 
    private bool b_SkillShot_Timer; // use to enable or disable skillshot timer
    ...
    private int Led_Mission_InProgress_State;
    private int Led_Part1_InProgress_State;

    private int Step; // Used on function Counter(). Represent the different step to reach a mission 
    private int Step_MultiBall; // use when you need to pass through rollover Grp2 in a specific order. 
    ...
    private void Update()
    {
        ...
        if (!b_Pause)
        {
            ...
            if (inputManager != null && Step < HowManyTime_Gpr1 && Rollover_Type3_Grp_1 && init_Ended)
            {
                ...
                if (inputManager.WasRightFlipperPressed() && Rollover_StopMoving) Move_Leds_To_Right();
                // Left flipper moves LEDs left
                if (inputManager.WasLeftFlipperPressed() && Rollover_StopMoving) Move_Leds_To_Left();
            }
            ...
            if (Rollover_Type3_Grp_1)
                ...
                {
                    if (touch.phase == TouchPhase.Began && Rollover_StopMoving)
                    {
                        ...
                        if (normalizedX > 0.5f && normalizedY < 0.5f) Move_Leds_To_Right();

                        // Left side of screen = move left
                        if (normalizedX < 0.5f && normalizedY < 0.5f) Move_Leds_To_Left();
                    }
                }
            ...
            if (b_MissionTimer)
            {
                ...
                if (missionTimer == 0)
                {
                    // if missionTimer == 0 the mission is failed
            ...
            if (b_SkillShot_Timer)
            {
                // Skillshot MISSION TIMER
                missionTimer = Mathf.MoveTowards(missionTimer, 0, // Timer progression
                    Time.deltaTime);
    ...
    public void Bonus_text()
    {
        // --> Text on LCD screen when mission is complete
        if (ExtraBall)
        {
            ...
            if (gameManager.F_return_multiplier() <= 10)
            {
                if (Mission_Txt.Length > 2 && Mission_Txt[0] != "" && Mission_Txt[2] != "")
                    gameManager.Add_Info_To_Array(Mission_Txt_name + "\n" + "<size= 20>"
                                                  ...
                                                  + gameManager.F_return_multiplier() // text : x
                                                  + "</size>", 3);
            }
            else
            {
                // SUper Bonus if multiplier > 10
                if (Mission_Txt.Length > 3 && Mission_Txt[0] != "" && Mission_Txt[3] != "")
                    gameManager.Add_Info_To_Array(Mission_Txt_name + "\n" + "<size= 20>"
                                                  + Mission_Txt[0] + "\n" // text : Mission Complete 
                                                  + Mission_Txt[3] + " " // text : Super Bonus
                                                  + gameManager.F_return_Mulitplier_SuperBonus()
                                                  + "</size>", 3);
            }
        }
        else if (KickBack)
    ...
    public void Counter(int num)
    {
        ...
        if (!b_Pause)
        {
            // USE when the game PAUSE MODE or When you want to pause a mission IN GAME
            if (Grp_1_Target)
                Part_1_Type_Target_Gpr1(num); // Part 1 : Pre mission
            else if (Grp_1_Rollover)
                Part_1_Type_Rollover_Gpr1(num);
            else if (Grp_1_Bumper)
                Part_1_Type_Bumper_Gpr1(num);
            else if (Grp_1_Spinner)
                Part_1_Type_Spinner_Gpr1(num);
            else if (Grp_1_Hole) Part_1_Type_Hole_Gpr1(num);

            if (Grp_2_Target)
                Part_2_Type_Target_Gpr2(num); // Part 2 : Mission start
            else if (Grp_2_Rollover)
                Part_2_Type_Rollover_Gpr2(num);
            else if (Grp_2_Bumper)
                Part_2_Type_Bumper_Gpr2(num);
            else if (Grp_2_Spinner)
                Part_2_Type_Spinner_Gpr2(num);
            else if (Grp_2_Hole)
                Part_2_Type_Hole_Gpr2(num);
            else if (obj_Grp_2.Length == 0 || HowManyTime_Gpr2 == 0) // If no object on part 2.
                if (Step == HowManyTime_Gpr1)
                    Step++;

            if (Grp_2_Target)
                Part_3_Type_Target_Gpr2(num); // Part 3 : Mission				
            else if (Grp_2_Rollover)
                Part_3_Type_Rollover_Gpr2(num);
            else if (Grp_2_Bumper)
                Part_3_Type_Bumper_Gpr2(num);
            else if (Grp_2_Spinner)
                Part_3_Type_Spinner_Gpr2(num);
            else if (Grp_2_Hole) Part_3_Type_Hole_Gpr2(num);


            if (Step == HowManyTime_Gpr1 + HowManyTime_Gpr2 + 1)
            {
                // Part 4 : Mission Complete
                ...
                else
                {
                    if (ExtraBall)
                    {
                        // Options when the mission is complete	
                    ...
                    else if (KickBack)
                    {
                        ...
                        for (var i = 0; i < obj_Led_Kickback.Length; i++) Kickback_Led[i].F_ChangeSprite_On();
                        Mission_Complete();
                    }
                    else
                    {
    ...
    public void Disable_Skillshot_Mission()
    {
        // --> Call by the script Game_Manager.js on object Game_Manger on the hierarchy. Send when the player lose a new ball
    ...
    public void Enable_Skillshot_Mission()
    {
        // --> Call by the script Game_Manager.js on object Game_Manger on the hierarchy. Send when the player start a new ball
    ...
    public void F_First_Hole_Grp1()
    {
        ...
        for (var i = 0; i < obj_Grp_1.Length; i++) Hole_Grp_1[i] = obj_Grp_1[i].GetComponent<Hole>(); // Connect the Mission to obj_Grp_1[i].GetComponent<Target>();	 component
    }

    public void F_First_Hole_Grp2()
    {
        Hole_Grp_2 = new Hole[obj_Grp_2.Length];
        for (var i = 0; i < obj_Grp_2.Length; i++) Hole_Grp_2[i] = obj_Grp_2[i].GetComponent<Hole>(); // Connect the Mission to obj_Grp_1[i].GetComponent<Target>();	 component
    }

    public void F_First_Spinner_Grp1()
    {
        // init Spinner Grp1
        Spinner_Grp_1 = new Spinner_LapCounter[obj_Grp_1.Length];
        for (var i = 0; i < obj_Grp_1.Length; i++) Spinner_Grp_1[i] = obj_Grp_1[i].GetComponent<Spinner_LapCounter>(); // Connect the Mission to obj_Grp_1[i].GetComponent<Target>();	 component
    }

    public void F_First_Spinner_Grp2()
    {
        // init Spinner Grp2
        Spinner_Grp_2 = new Spinner_LapCounter[obj_Grp_2.Length];
        for (var i = 0; i < obj_Grp_2.Length; i++) Spinner_Grp_2[i] = obj_Grp_2[i].GetComponent<Spinner_LapCounter>(); // Connect the Mission to obj_Grp_1[i].GetComponent<Target>();	 component
    }

    public void F_First_Target_Grp1()
    {
        target_Grp_1 = new Target[obj_Grp_1.Length];
        for (var i = 0; i < obj_Grp_1.Length; i++) target_Grp_1[i] = obj_Grp_1[i].GetComponent<Target>(); // Connect the Mission to obj_Grp_1[i].GetComponent<Target>();	 component
    ...
    public void F_First_Target_Grp2()
    {
        ...
        for (var i = 0; i < obj_Grp_2.Length; i++) target_Grp_2[i] = obj_Grp_2[i].GetComponent<Target>(); // Connect the Mission to obj_Grp_1[i].GetComponent<Target>();	 component
    ...
    {
        var tmp = 0;
        if (obj_Grp_1.Length != obj_Grp_1_Leds.Length) tmp = obj_Grp_1.Length - obj_Grp_1_Leds.Length;

        num += obj_Grp_1.Length - tmp;
        return num;
    }

    public void F_LedState()
    {
        // --> Use to switch on or switch Off the leds from arr_led_State
    ...
    public void F_Random_Bonus()
    {
        // --> if Random_Bonus = true. The player win a random Bonus (ExtraBall,BallSaver,Multiplier,Points Only)
    ...
    public void F_SkillshotTimer(int seconds)
    {
        // --> Init and start the skillshot timer											
    ...
    public void first()
    {
        // --> Initialisation when scene start
        obj_Led = new GameObject[obj_Grp_1_Leds.Length + obj_Grp_2_Leds.Length]; // --> Put obj_Grp_1_Leds + obj_Grp_2_Leds inside obj_Led
        for (var l = 0; l < obj_Grp_1_Leds.Length; l++) obj_Led[l] = obj_Grp_1_Leds[l];
        for (var l = 0; l < obj_Grp_2_Leds.Length; l++) obj_Led[l + obj_Grp_1_Leds.Length] = obj_Grp_2_Leds[l];

        Led_Renderer = new ChangeSpriteRenderer[obj_Led.Length]; // --> Connect the Mission to obj_Led[i]<ChangeSpriteRenderer>() component. 
        for (var k = 0; k < obj_Led.Length; k++) Led_Renderer[k] = obj_Led[k].GetComponent<ChangeSpriteRenderer>();
        ...
        for (var i = 0; i < obj_Led.Length; i++) arr_led_State.Add(0); // Create Array to record the Leds state from obj_Led
        ...
        if (b_SkillShot)
        {
            // --> if b_SkillShot = true. This mission is used for the Skillshot.
            GameObject[] gos;
            gos = GameObject.FindGameObjectsWithTag("Plunger");
            foreach (var go in gos) go.SendMessage("Connect_Plunger_To_Skillshot_Mission", gameObject); // you need to tell the PLUNGER that is the skillshot mission	SpringLauncher.js	on gameObject SpringLauncher on the hierarchy

            gameManager.F_Init_Skillshot_Mission(gameObject); // you need to tell the ManagerGame that is the skillshot mission	 
        ...
        if (obj_Door_Kickback.Length > 0)
        {
            // Init kickback if object is connected to obj_Door_Kickback
            Kickback_Door = new Target[obj_Door_Kickback.Length];
            for (var i = 0; i < obj_Door_Kickback.Length; i++) Kickback_Door[i] = obj_Door_Kickback[i].GetComponent<Target>(); // Access Target  component
            Kickback_Led = new ChangeSpriteRenderer[obj_Led_Kickback.Length];
            for (var i = 0; i < obj_Led_Kickback.Length; i++) Kickback_Led[i] = obj_Led_Kickback[i].GetComponent<ChangeSpriteRenderer>(); // Access Target  ChangeSpriteRenderer
        }
        ...
        else if (Grp_1_Rollover)
        {
            ...
            for (var i = 0; i < obj_Grp_1.Length; i++) Rollover_Grp_1[i] = obj_Grp_1[i].GetComponent<Rollovers>();

            if (Rollover_Type3_Grp_1) HowManyTime_Gpr1 = obj_Grp_1.Length;
        }
        else if (Grp_1_Bumper)
        {
            // Grp1 : type : Bumper
        ...
        else if (Grp_2_Rollover)
        {
            ...
            for (var i = 0; i < obj_Grp_2.Length; i++) Rollover_Grp_2[i] = obj_Grp_2[i].GetComponent<Rollovers>();
        }
        else if (Grp_2_Bumper)
        {
            // Grp2 : type : Bumper
        ...
        if (Led_Mission_InProgress)
        {
            // Init the led : Led_Mission_InProgress
        ...
        {
            // Init the led : Led_Part1_InProgress
    ...
    public void Init_KickBack_WhenPlayerLoseBall()
    {
        // -> Init kickback when the ball is lost
        //Debug.Log("Here 3");
        if (BeginWithKickBack)
        {
            // init kickback
            for (var i = 0; i < obj_Door_Kickback.Length; i++) Kickback_Door[i].Desactivate_Object();
            for (var i = 0; i < obj_Led_Kickback.Length; i++) Kickback_Led[i].F_ChangeSprite_On();
        }
        else
        {
            // init kickback
            for (var i = 0; i < obj_Door_Kickback.Length; i++) Kickback_Door[i].Activate_Object();
            for (var i = 0; i < obj_Led_Kickback.Length; i++) Kickback_Led[i].F_ChangeSprite_Off();
        }
    }
    ...
    public void Mission_Intialisation()
    {
        // --> Function to initialize a mission
        init_Ended = false;
        if (Grp_1_Target)
            Mission_Intialisation_Target_Gp1();
        else if (Grp_1_Rollover)
            Mission_Intialisation_Rollover_Gp1();
        else if (Grp_1_Bumper)
            Mission_Intialisation_Bumper_Gp1();
        else if (Grp_1_Spinner)
            Mission_Intialisation_Spinner_Gp1();
        else if (Grp_1_Hole) Mission_Intialisation_Hole_Gp1();

        if (Grp_2_Target)
            Mission_Intialisation_Target_Gp2();
        else if (Grp_2_Rollover)
            Mission_Intialisation_Rollover_Gp2();
        else if (Grp_2_Bumper)
            Mission_Intialisation_Bumper_Gp2();
        else if (Grp_2_Spinner)
            Mission_Intialisation_Spinner_Gp2();
    ...
    public void Mission_Intialisation_AfterBallLost()
    {
        // --> initialisation after a ball lost
    ...
    public void Mission_Intialisation_Bumper_Gp1()
    {
        // Init Led Mission
    ...
    // PART INITIALISATION
    public void Mission_Intialisation_Bumper_Gp2()
    {
        // --> Init Leds Mission Bumper
        if (Led_Mission_InProgress)
        {
            // init Led_Mission_InProgress
            led_Mission_InProgress_.F_ChangeSprite_Off();
            Led_Mission_InProgress_State = 0;
            led_Mission_InProgress_.led_Part_InProgress_State(0);
        }
    ...
    public void Mission_Intialisation_Rollover_Gp1()
    {
        // --> Rollover : Init Gpr_1
        for (var j = 0; j < obj_Grp_1_Leds.Length; j++)
        {
        ...
        else if (Rollover_Order_Grp_1)
        {
            // --> Rollover_Order_Grp_1
            for (var i = 0; i < obj_Grp_1_Leds.Length; i++)
            {
                // init obj_Grp_1_Leds state 
                if (i == 0)
    ...
    // PART INITIALISATION
    public void Mission_Intialisation_Rollover_Gp2()
    {
        // --> Init Rollover Grp2
        if (Led_Mission_InProgress)
        {
    ...
    public void Mission_Intialisation_Spinner_Gp1()
    {
        // Init Mission Led Grp1
        for (var j = 0; j < obj_Grp_1_Leds.Length; j++)
        {
            // Init Gpr_1
            arr_led_State[j] = 0; // You want switch Off the leds
    ...
    // PART INITIALISATION	
    public void Mission_Intialisation_Spinner_Gp2()
    {
        // --> Init Leds Mission Grp2
        if (Led_Mission_InProgress)
        {
            // init Led_Mission_InProgress
            led_Mission_InProgress_.F_ChangeSprite_Off();
    ...
    public void Mission_Intialisation_StartGame()
    {
        // --> Function to initialize a mission when a game is starting. Call by ManagerGame.js
        init_Ended = false;
        if (Grp_1_Target)
            Mission_Intialisation_Target_Gp1();
        else if (Grp_1_Rollover)
    ...
    {
        for (var j = 0; j < obj_Grp_1.Length; j++)
        {
            // Init Gpr_1
            if (Target_No_Order_Grp_1)
            {
                // -> Target_No_Order_Grp_1 
                arr_obj_Grp_1_State[j] = 0;
                target_Grp_1[j].Activate_Object();
            }
            else if (Target_Order_Grp_1)
            {
                // -> Target_Order_Grp_1
                if (j == 0)
                {
    ...
    public void Mission_Intialisation_Target_Gp2()
    {
        ...
        if (Target_Type_Stationary) // -> if Target_Type_Stationary
            for (var j = 0; j < obj_Grp_2.Length; j++)
            {
                // Init Gpr_2	
    ...
    public void MissionTimer(int seconds)
    {
    ...
    public void Mode_MultiBall_Ended()
    {
        // --> Call by manager_Game on the hierachy
        Play_LedAnim_ObjAnim_LCDAnim_Fail(); // Play led animation; object animation or lcd animation							
        if (Mission_Txt.Length > 1
    ...
    public void Move_Leds_To_Left()
    {
        // --> use for Rollover_type_3. Leds move from Right to the Left when the player press Right Flipper 
    ...
    public void Move_Leds_To_Right()
    {
        // --> use for Rollover_type_3. Leds move from Left to the right when the player press Left Flipper 
        Rollover_StopMoving = false;
        LedTmp = new int[obj_Grp_1.Length];
        for (var i = 0; i < obj_Grp_1.Length; i++) // Record the new possition of the leds.
    ...
    public void newTxtArray()
    {
        // --> Init Mission Text : if(Mission_Txt.Length == 0) Create text for the mission 
    ...
    // PART 1
    public void Part_1_Type_Bumper_Gpr1(int num)
    {
        // --> Bumper Section Part 1
        for (var i = 0; i < obj_Grp_1.Length; i++)
        {
            if (Bumper_Grp_1[i].index_info() == num)
                if (Step < HowManyTime_Gpr1)
                {
                    if (arr_led_State.Count >= HowManyTime_Gpr1 && obj_Grp_1_Leds.Length > 0)
                    {
                        // if the Bumper as leds attached
        ...
        for (var i = 0; i < obj_Grp_1.Length; i++)
        {
            if (Hole_Grp_1[i].index_info() == num)
                if (Step < HowManyTime_Gpr1)
                {
                    if (arr_led_State.Count >= HowManyTime_Gpr1 && obj_Grp_1_Leds.Length > 0)
                    {
                        // if the Rollover as leds attached
                        arr_led_State[Step] = 1; // You want switch On the leds
                        Led_Renderer[Step].F_ChangeSprite_On(); // Led : On	
                    }
    ...
    // PART 1
    public void Part_1_Type_Rollover_Gpr1(int num)
    {
        // --> Part 1
        if (Step < HowManyTime_Gpr1)
            for (var i = 0; i < obj_Grp_1.Length; i++)
            {
                                                                                                     ...
                                                                                                     && Rollover_No_Order_Grp_1 && Rollover_Grp_1[i].index_info() == num)
                {
                    ...
                    {
                        // if the rollover as a led attached
                ...
                if (Rollover_No_Order_Grp_1 && Rollover_Grp_1[i].index_info() == num)
                {
                    // -> Case 2
                    Step++; // Next step	

                    if (SpecificText) Txt_Rollover_type_3(); // Write a text on LCD Screen
                    else if (Mission_Txt.Length > 4 && Mission_Txt[4] != "") gameManager.Add_Info_To_Array(Mission_Txt_name + "\n" + Mission_Txt[4] + (HowManyTime_Gpr1 - Step), 3);
                ...
                if (Rollover_Order_Grp_1 // -> Case 3 
                    && Rollover_Grp_1[Step % obj_Grp_1.Length].index_info() == num)
                {
                    ...
                    if (Step < HowManyTime_Gpr1)
                    {
                        ...
                        if (obj_Grp_1_Leds.Length > Step % obj_Grp_1.Length)
                        {
                            // If a led is attached to the target
                ...
                if (Rollover_Type3_Grp_1 // -> Case 4 
                    && Rollover_Grp_1[i].index_info() == num && arr_led_State[i] == 0)
                {
                    if (arr_led_State.Count > F_Led_Gpr1_num(Step))
                    {
                        // if the rollover as a led attached
    ...
    // PART 1
    public void Part_1_Type_Spinner_Gpr1(int num)
    {
        // --> Spinner Part 1
        ...
        for (var i = 0; i < obj_Grp_1.Length; i++)
        {
            if (Spinner_Grp_1[i].index_info() == num)
                if (Step < HowManyTime_Gpr1)
                {
                    if (arr_led_State.Count >= HowManyTime_Gpr1 && obj_Grp_1_Leds.Length > 0)
                    {
                        // if the Spinner as leds attached
                        arr_led_State[Step] = 1; // You want switch On the leds
                        Led_Renderer[Step].F_ChangeSprite_On(); // Led : On	
                    }
    ...
    public void Part_1_Type_Target_Gpr1(int num)
    {
        if (Step < HowManyTime_Gpr1) // --> PART 1 : Shoot target Gpr_1 : Steps to be performed by the player before the mission begins
            ...
            {
                if (target_Grp_1[i].index_info() == num && arr_obj_Grp_1_State[i] == 0)
                {
                    // Target from target_Grp_1 and the target has not been touched
                    arr_obj_Grp_1_State[i] = 1; // Update the state of target from Gpr_1


                    if (arr_led_State.Count > F_Led_Gpr1_num(i))
                    {
                        // if the taget as a led attached
    ...
    // PART 2
    public void Part_2_Type_Bumper_Gpr2(int num)
    {
        // --> Bumper Part 2
        if (Step == HowManyTime_Gpr1)
        {
            ...
            for (var i = 0; i < obj_Grp_1_Leds.Length; i++)
            {
                ...
                {
                    // Keep Led Grp1 On During Mission if true
    ...
    // PART 2
    public void Part_2_Type_Rollover_Gpr2(int num)
    {
        // --> Rollover Part 2
        if (Step == HowManyTime_Gpr1)
        {
            ...
            for (var i = 0; i < obj_Grp_1_Leds.Length; i++)
            {
                ...
                if (KeepLedGrp1OnDuringMission)
                {
                    // Keep Led On from obj_Grp1_Leds during mission progress
                    arr_led_State[F_Led_Gpr1_num(i)] = 1;
                    Led_Renderer[F_Led_Gpr1_num(i)].F_ChangeSprite_On();
                }
                else
                {
                    // Switch Off leds from obj_Grp1_Leds
                    arr_led_State[F_Led_Gpr1_num(i)] = 0;
                    Led_Renderer[F_Led_Gpr1_num(i)].F_ChangeSprite_Off();
                }
            }
            ...
            if (multiBall)
            {
                // -> Start multiball if multiBall = true									
                gameManager.F_Mission_MultiBall(missionIndex.F_index(), numberOfBall);
    ...
    // PART 2
    public void Part_2_Type_Spinner_Gpr2(int num)
    {
        // --> Part 2
        if (Step == HowManyTime_Gpr1)
        {
    ...
    public void Part_2_Type_Target_Gpr2(int num)
    {
        if (Step == HowManyTime_Gpr1)
        {
            ...
            if (Target_No_Order_Grp_2 || Target_Type_Stationary) // -> Target_No_Order_Grp_2 || 	Target_Type_Stationary	
                for (var j = 0; j < obj_Grp_2.Length; j++)
                {
                    // Update the state of target from Gpr_2										
    ...
    // PART 3
    public void Part_3_Type_Bumper_Gpr2(int num)
    {
        // Bumper Part 3
        for (var i = 0; i < obj_Grp_2.Length; i++)
        {
            if (Bumper_Grp_2[i].index_info() == num)
                if (Step > HowManyTime_Gpr1 && Step < HowManyTime_Gpr1 + HowManyTime_Gpr2 + 1)
                {
                    ...
                    if (obj_Grp_2_Leds.Length > Step - 1 - HowManyTime_Gpr1)
                    {
                        // if the rollover as a led attached
    ...
    // PART 3
    public void Part_3_Type_Rollover_Gpr2(int num)
    {
        // Part 3 : Rollover
        for (var i = 0; i < obj_Grp_2.Length; i++)
        {
            if (Step > HowManyTime_Gpr1 && Step <= HowManyTime_Gpr1 + HowManyTime_Gpr2)
            {
                if (multiBall)
                {
                    // if mode multi ball true
                    if (Rollover_No_Order_Grp_2 && Rollover_Grp_2[i].index_info() == num)
                    {
                        // Case 1
                        if (b_Mission_Multi_Timer) MissionTimer(Mission_Time); // Init Timer with a value
                    ...
                    if (Rollover_Order_Grp_2 && Rollover_Grp_2[(Step_MultiBall - 1 - HowManyTime_Gpr1) % obj_Grp_2.Length].index_info() == num)
                    {
                        // Case 2
                        if (b_Mission_Multi_Timer) MissionTimer(Mission_Time); // Init Timer with a value	
                ...
                {
                    // --> if mode multi ball = false
                    if (Rollover_Grp_2.Length == 1 && Rollover_No_Order_Grp_2 // -> case 1
                                                   && Rollover_Grp_2[(Step - 1 - HowManyTime_Gpr1) % obj_Grp_2.Length].index_info() == num)
                    {
                        // -> if there is only one rollover on Rollover_Grp_2. 
                        if (b_Mission_Multi_Timer) MissionTimer(Mission_Time); // Init Timer with a value
                    ...
                    if (Rollover_Order_Grp_2 && Rollover_Grp_2[(Step - 1 - HowManyTime_Gpr1) % obj_Grp_2.Length].index_info() == num)
                    {
                        // -> case 3	
                        if (b_Mission_Multi_Timer) MissionTimer(Mission_Time); // Init Timer with a value	
    ...
    // PART 3
    public void Part_3_Type_Spinner_Gpr2(int num)
    {
        // Part 3
        ...
        for (var i = 0; i < obj_Grp_2.Length; i++)
        {
            if (Spinner_Grp_2[i].index_info() == num)
                ...
                if (Step > HowManyTime_Gpr1 && Step < HowManyTime_Gpr1 + HowManyTime_Gpr2 + 1)
                {
                    ...
                    if (obj_Grp_2_Leds.Length > Step - 1 - HowManyTime_Gpr1)
                    {
                        // if the spinner as a led attached
                        arr_led_State[obj_Grp_1_Leds.Length + (Step - 1 - HowManyTime_Gpr1)] = 1; // You want switch off the leds
        ...
        if (Step > HowManyTime_Gpr1 && Step < HowManyTime_Gpr1 + HowManyTime_Gpr2 + 1)
            for (var i = 0; i < obj_Grp_2.Length; i++)
            {
                if (target_Grp_2[i].index_info() == num && arr_obj_Grp_2_State[i] == 0)
                {
                    ...
                    if (Target_Order_Grp_2 && Step < HowManyTime_Gpr1 + obj_Grp_2.Length + 1)
                    {
                        // -> if Target_Order_Grp_2 is checked
                        ...
                        if (obj_Grp_2_Leds.Length > Step - HowManyTime_Gpr1 - 1)
                        {
                            // If a led is attached to the target
                    ...
                    if (Mission_Txt.Length > 5 && Mission_Txt[5] != "")
                        // Write a text on LCD Screen
                        gameManager.Add_Info_To_Array(Mission_Txt_name + "\n" + Mission_Txt[5] + (HowManyTime_Gpr1 + obj_Grp_2.Length + 1 - Step), 3);
                    if (Step < HowManyTime_Gpr1 + HowManyTime_Gpr2)
    ...
    public void Pause_Game_Mission()
    {
        // Mission : PAUSE (the scene is on Pause)
        if (!b_Pause)
        {
            b_Pause = true; // Pause mission : on
            if (sound_.isPlaying)
            {
                // If a sound is playing when pause start
    ...
    {
        // What To Do When Pause START. Call Pause_Mission.js on mission's game object. All the led's missions are switch off
        if (b_PauseMissionMode)
        {
            ...
            for (var i = 0; i < Led_Renderer.Length; i++) Led_Renderer[i].F_ChangeSprite_Off();
            if (Led_Mission_InProgress) led_Mission_InProgress_.F_ChangeSprite_Off();
            if (Led_Part1_InProgress) led_Part1_InProgress_.F_ChangeSprite_Off();
        }
    }

    public void Pause_Stop()
    {
        // Waht To Do When pause STOP. Call Pause_Mission.js on mission's game object
        if (b_PauseMissionMode)
        {
            if (obj_Gui_Debug)
            ...
            if (Grp_1_Target)
                ...
                Pause_Stop_Rollover_Gpr1();
            else if (Grp_1_Bumper)
                Pause_Stop_Bumper_Gpr1();
            else if (Grp_1_Spinner)
                Pause_Stop_Spinner_Gpr1();
            else if (Grp_1_Hole) Pause_Stop_Hole_Gpr1();

            if (Grp_2_Target)
                Pause_Stop_Target_Gpr2();
            else if (Grp_2_Rollover)
                Pause_Stop_Rollover_Gpr2();
            else if (Grp_2_Bumper)
                Pause_Stop_Bumper_Gpr2();
            else if (Grp_2_Spinner)
                Pause_Stop_Spinner_Gpr2();
    ...
    public void Play_LCD_Screen_Animation(int num)
    {
        ...
        foreach (var go in gos) Destroy(go); // You could play only one animation
    ...
    public void Play_LedAnim_ObjAnim_LCDAnim_Part3()
    {
        ...
        if (sfx_Part3)
        {
            sound_.clip = sfx_Part3;
            sound_.Play();
        }
    }

    public void Skillshot_Mission()
    {
        // --> Call by the script SpringLauncher.js on the object Spring on the hierarchy.
        if (b_Mission_SkillShot && !b_Skillshot_OnAir)
        {
            // Use to prevent the skillshot timer restart if the ball is not correctly ejected from the plunger				
            F_SkillshotTimer(SkillshotDuration); // start the skillshot timer
            b_Skillshot_OnAir = true; // The timer can not start twice
        }
    }

    public void Skillshot_Mission_Complete()
    {
        // --> Skillshot mission is complete
    ...
    public void Skillshot_Mission_Fail()
    {
        // --> Skillshot mission is failed
    ...
    public void Txt_Rollover_type_3()
    {
        // --> use for Rollover on obj_Gpr1. Display Mission_Txt[11] on LCD screen 
        if (Mission_Txt.Length > 11 && obj_Grp_1.Length > 1 && Mission_Txt[11] != "")
        {
            // -> use for Rollover_type_3.			
            var tmp_Txt = "";
            for (var i = 0; i < obj_Grp_1.Length; i++)
            {
        ...
        else if (Mission_Txt.Length > 11 && obj_Grp_1.Length == 1)
        {
            // -> use for Rollover_No_Order_Grp_1 and Rollover_Order_Grp_1.
            var tmp_Txt = "";
            for (var i = 0; i < obj_Grp_1_Leds.Length; i++)
            {
                if (arr_led_State[i] == 0) // if led state == 0 the letter is display with this color 

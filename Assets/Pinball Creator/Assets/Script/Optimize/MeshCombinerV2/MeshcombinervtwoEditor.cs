...
public class MeshcombinervtwoEditor : Editor {
	...
	public override void OnInspectorGUI(){
		...
		EditorGUILayout.HelpBox("(Optional) : Exclude gameObjects with specific TAG",MessageType.Info);
			EditorGUILayout.LabelField("");
		if (GUILayout.Button("Reset tag"))
		{
			list_Tags.ClearArray();
			for(var i =0;i<6;i++)
				list_Tags.InsertArrayElementAtIndex(0);

			list_Tags.GetArrayElementAtIndex(0).stringValue = "Flipper";
			list_Tags.GetArrayElementAtIndex(1).stringValue = "NoCombineMesh";
			list_Tags.GetArrayElementAtIndex(2).stringValue = "Plunger";
			list_Tags.GetArrayElementAtIndex(3).stringValue = "AnimatedObject";
			list_Tags.GetArrayElementAtIndex(4).stringValue = "spinner";
			list_Tags.GetArrayElementAtIndex(5).stringValue = "Leds";
		}
		if (GUILayout.Button("Add new tag", GUILayout.Height(30)))
			{
				b_AddTag = true;
			}
			EditorGUILayout.LabelField("");
